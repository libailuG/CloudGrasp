...
def main():
    with (LOG / 'pick.lock').open('w') as lock:
        ...
        if subprocess.call(['bash', str(HERE / 'ready.sh'), '--timeout', '15']):
            return 1
        ...
        try:
            with Path(str(run) + '-monitor.log').open('w') as monitor, Path(str(run) + '.log').open('w') as log:
                watcher = subprocess.Popen(['/usr/bin/python3', str(HERE / 'watch_cube.py'), '300', str(report_path)], stdout=monitor, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL)
                launch = subprocess.Popen(['ros2', 'launch', 'cloudgrasp_sim', 'pick.launch.py', *sys.argv[1:]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL, text=True, start_new_session=True)
