...
case "$command" in
  ...
  *) echo 'Usage: bash learning/rebuild/lab.sh start 1..7 [gui:=false] [rviz:=false] | stop | ready 1..7 | joint wrist|home|open|close | observe | move [execute:=true] [target:=home] | scene table|add|block|remove|list | perceive | reset | pick' ;;
esac
